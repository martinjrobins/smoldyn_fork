...
double measurephi(double *low,double *high,double *spheres,int n,int dim,int itmax);
...
double measurephi(double *low,double *high,double *spheres,int n,int dim,int itmax) {
	...
	for(it=0;it<itmax;it++) {
		...
		for(i=0;i<n && !hit;i++) {
			...
			if(dist<=spheres[4*i+dim]*spheres[4*i+dim]) hit=1; }
		count+=hit; }
