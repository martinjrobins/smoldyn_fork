...
int dosurfinteract(simptr sim,moleculeptr mptr,int ll,int m,panelptr pnl,enum PanelFace face,double *crsspt) {
	...
	else if(act==SArevdes || act==SAirrevdes) {		// reversible desorb
...
int checksurfacebound(simptr sim,int ll) {
	...
	for(m=0;m<nmol;m++) {
		mptr=mlist[m];
		if(mptr->mstate!=MSsoln) {
			er=dosurfinteract(sim,mptr,ll,m,mptr->pnl,PFnone,mptr->posx);
			if(er==-1) simLog(sim,10,"Unable to allocate memory in dosurfinteract"); }}
	return 0; }
