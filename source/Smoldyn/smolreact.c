	...
	if(rxnss) {
		for(r=0;r<rxnss->totrxn;r++) {
			...
			if(rxn->bindrad2<0) {
				...
				if(rxn->rparamt==RPconfspread) simLog(sim,7," WARNING: confspread radius for order 2 reaction %s is larger than box dimensions\n",rxn->rname);
				else if(rxn->rparamt==RPbounce) simLog(sim,7," WARNING: bounce radius for order 2 reaction %s is larger than box dimensions\n",rxn->rname);
				else simLog(sim,7," WARNING: binding radius for order 2 reaction %s is larger than box dimensions\n",rxn->rname);
				error++; }
