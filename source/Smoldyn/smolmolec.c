...
int molsetsurfdrift(simptr sim,int ident,enum MolecState ms,int surface,enum PanelShape ps,double *drift) {
	...
	if(ms!=MSall) mshi=MolecState((mslo=ms)+1);
	else {mslo=MolecState(1);mshi=MolecState(MSMAX);}
	...
	while(1) {
		if(ident>0) {						// single species
			...
			if(++i1==sim->mols->nspecies) break; }
		else if(ident==-6) {		// wildcard selected species
			if((i1=molwildcardname(sim->mols,NULL,0,0))<0) break; }

		if(!mols->surfdrift[i1]) {
			CHECKMEM(mols->surfdrift[i1]=(double****) calloc(MSMAX,sizeof(double***)));
			for(ms1=(MolecState)0;ms1<MSMAX;ms1=(MolecState)(ms1+1)) mols->surfdrift[i1][ms1]=NULL; }
		for(ms1=mslo;ms1<mshi;ms1=MolecState(ms1+1)) {
			if(!mols->surfdrift[i1][ms1]) {
				CHECKMEM(mols->surfdrift[i1][ms1]=(double***) calloc(sim->srfss->maxsrf,sizeof(double**)));
				for(s1=0;s1<sim->srfss->maxsrf;s1++) mols->surfdrift[i1][ms1][s1]=NULL; }
			for(s1=slo;s1<shi;s1++) {
				if(!mols->surfdrift[i1][ms1][s1]) {
					CHECKMEM(mols->surfdrift[i1][ms1][s1]=(double**) calloc(PSMAX,sizeof(double*)));
					for(ps1=(PanelShape)0;ps1<PSMAX;ps1=(PanelShape)(ps1+1)) mols->surfdrift[i1][ms1][s1][ps1]=NULL; }
				for(ps1=pslo;ps1<pshi;ps1=(PanelShape)(ps1+1)) {
					if(!mols->surfdrift[i1][ms1][s1][ps1]) {
						CHECKMEM(mols->surfdrift[i1][ms1][s1][ps1]=(double*) calloc(dim-1,sizeof(double)));
						for(d=0;d<dim-1;d++) mols->surfdrift[i1][ms1][s1][ps1][d]=0; }

					for(d=0;d<dim-1;d++)
						mols->surfdrift[i1][ms1][s1][ps1][d]=drift[d]; }}}}
						...
						if(oldsurfdrift[i][ms][s])
							for(ps=(PanelShape)0;ps<PSMAX;ps=(PanelShape)(ps+1))
								if(oldsurfdrift[i][ms][s][ps])
									CHECK(molsetsurfdrift(sim,i,ms,s,ps,oldsurfdrift[i][ms][s][ps])==0);
...
void molssoutput(simptr sim) {
	...
	for(i=1;i<nspecies;i++) {
		...
		if(mols->surfdrift && mols->surfdrift[i]) {
			...
			for(ms=(MolecState)1;ms<MSMAX;ms=(MolecState)(ms+1))
